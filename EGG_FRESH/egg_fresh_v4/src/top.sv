...
module top (
    ...
    // ---- TẠO CÁC DÂY NỐI MỚI ----
    wire [15:0] w_ch_raw [0:5];
    wire        w_ai_start, w_ai_done;
    wire [7:0]  w_ai_score;

    // ---- SỬA MODULE u_as7263 (Thêm các cổng mới) ----
    as7263_driver u_as7263 (
        ...
        // Thêm kết nối AI vào đây:
        .o_ai_start ( w_ai_start ),
        .i_ai_done  ( w_ai_done  ),
        .i_ai_score ( w_ai_score ),
        .o_ch_raw   ( w_ch_raw   )
    );

    // ---- THÊM MODULE MLP VÀO ĐÂY ----
    mlp_inference u_mlp (
        .clk            ( sys_clk    ),
        .rst_n          ( rst_n      ),
        .i_start        ( w_ai_start ),
        .i_raw_channels ( w_ch_raw   ),
        .o_score        ( w_ai_score ),
        .o_done         ( w_ai_done  )
    );
